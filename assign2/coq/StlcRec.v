...
Inductive step : tm -> tm -> Prop :=
  ...
  | ST_Rec_Step : forall tb ts ta ta' T,
      step ta ta' ->
      step (tm_rec_num tb ts ta T) (tm_rec_num tb ts ta' T)
  | ST_Rec_Base : forall tb ts T,
      step (tm_rec_num tb ts (tm_num 0) T) tb
  | ST_Rec_Dec : forall tb ts n T,
      step 
        (tm_rec_num tb ts (tm_num (S n)) T) 
        (bind_top (tm_rec_num tb ts (tm_num n) T)
          (bind_top (tm_num (S n)) ts)).
...
Inductive has_type : context -> tm -> ty -> Prop :=
  ...
  | T_Rec : forall ctx tb ts ta T,
      has_type ctx tb T ->
      has_type (cons Ty_Num (cons T ctx)) ts T ->
      has_type ctx ta Ty_Num ->
      has_type ctx (tm_rec_num tb ts ta T) T.
...
Proof.
  ...
  - apply T_Rec.
    + apply IHHt1. reflexivity.
    + specialize (IHHt2 (cons Ty_Num (cons T pctx))). simpl in IHHt2.
      apply IHHt2. reflexivity.
...
Proof.
  ...
  - apply T_Rec.
    + eapply IHHtt2_1; eauto.
    + specialize (IHHtt2_2 ctx1 (cons Ty_Num (cons T pctx))). simpl in *.
      unfold shift_up in *. rewrite shiftf_plus_add.
      eapply IHHtt2_2; eauto.
    + eapply IHHtt2_3; eauto.
Qed.
...
Proof.
  ...
  - inversion Hs; subst.
    + apply T_Rec.
      ...
      * apply IHHt3; auto.
    + assumption.
    + eapply bind_top_preserve_typing.
      * apply T_Rec; auto. apply T_Num.
      * eapply bind_top_preserve_typing; eauto. apply T_Num.
Qed.
...
Proof.
  ...
  - right. specialize (IHHt3 eq_refl). specialize (IHHt1 eq_refl).
    destruct IHHt3 as [Hvt3 | [t3' Hst3]].
    + destruct (canonical_value_num _ Ht3 Hvt3) as [v3 Ev3].
      subst ta. destruct v3.
      * exists tb. apply ST_Rec_Base.
      * eexists. apply ST_Rec_Dec.
    + exists (tm_rec_num tb ts t3' T). apply ST_Rec_Step. auto.
Qed.
